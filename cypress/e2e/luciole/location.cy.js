...
describe("let's test the location", function () {
  ...
  it('tests the loc.get_key function', function () {
    let _pipe = $loc.get_key(new $loc.Hash())
    $should.equal(_pipe, '')
    let _pipe$1 = $loc.get_key(new $loc.Host())
    $should.equal(_pipe$1, 'example.cypress.io')
    let _pipe$2 = $loc.get_key(new $loc.Hostname())
    $should.equal(_pipe$2, 'example.cypress.io')
    let _pipe$3 = $loc.get_key(new $loc.Href())
    $should.equal(_pipe$3, 'https://example.cypress.io/commands/traversal')
    let _pipe$4 = $loc.get_key(new $loc.Origin())
    $should.equal(_pipe$4, 'https://example.cypress.io')
    let _pipe$5 = $loc.get_key(new $loc.Pathname())
    $should.equal(_pipe$5, '/commands/traversal')
    let _pipe$6 = $loc.get_key(new $loc.Port())
    $should.equal(_pipe$6, '')
    let _pipe$7 = $loc.get_key(new $loc.Protocol())
    $should.equal(_pipe$7, 'https:')
    let _pipe$8 = $loc.get_key(new $loc.Search())
    $should.equal(_pipe$8, '')
    let _pipe$9 = $loc.get_key(new $loc.SuperDomain())
    $should.equal(_pipe$9, 'cypress.io')
    let _pipe$10 = $loc.get_key(new $loc.SuperDomainOrigin())
